...
function k8s.list_containers_by_pod {
    {
        echo -n "${BOLD}NAME\t${BOLD}${WHITE}CONTAINER${NORMAL}\t${BOLD}STATUS\tAGE\tRESTARTS\tIP\tNODE\tNODE_IP${NORMAL}\n";
        {
            kub get pods -o json | \
            jq -r --arg RES "$NORMAL" --arg BLUE "$BLUE" \
            '.items[] | .metadata.name as $pod_name |  .spec.nodeName as $node | .status.hostIP as $nodeip | .status.podIP as $podip | .status.containerStatuses[] | [
                ...
                $BLUE + .name + $RES,
                (
                  if .state.waiting then (.state.waiting.reason // "Waiting")
