...
function tf
{
    ...
    >&2 echo "│ Set WORKSPACE to $TF_WORKSPACE"
    ...
    if [[ $1 =~ (apply) ]]; then
        ...
        if [[ -n $TF_COMMAND && "$*" != "${TF_COMMAND}" ]]; then
            >&2 echo "│ WORKSPACE=$TF_WORKSPACE  previously used with args: '$TF_COMMAND'"
            unset response
        ...
        if [[ -n $TF_OS_REGION_NAME && $OS_REGION_NAME != $TF_OS_REGION_NAME ]]; then
            >&2 echo "│ OS_REGION_NAME is not set to = $TF_OS_REGION_NAME "
            unset response
            [[ $SHELL =~ zsh ]] && \
