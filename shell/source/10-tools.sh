...
command -v bluetoothctl &> /dev/null && source <(bluetooth-pair --complete)
...
function git
{
    ...
    # push: -f => --force-with-lease, confirmation avant tout force push
    if [[ $1 = "push" ]]; then
        shift
        local force=0
        local -a args
        ...
        for a in "$@"; do
            case "$a" in
                -f) args+=(--force-with-lease); force=1 ;;
                --force|--force-with-lease*) args+=("$a"); force=1 ;;
                *) args+=("$a") ;;
            esac
        done

        if [[ $force -eq 1 ]]; then
            echo "CMD: git push ${args[*]}"
            local response
            [[ $SHELL =~ zsh ]] && vared -p 'Force push ? [y] ' -c response || read -r -p 'Force push ? [y] ' response
            [[ $response != y ]] && return 1
        fi
...
function ssh.gpg_agent_fwd_get_options
{
    local user_at_host=$1
    local key=$2

    if [[ -z $user_at_host || -z $key ]]; then
        echo "ssh.gpg_agent_fwd_get_options <user@host> <gpg key id/mail..>" >&2
        return 1
    fi
    ...
    function init_gpg_remote
    {
        ...
        grep -qx no-autostart ~/.gnupg/gpg-agent.conf 2> /dev/null \
            || echo no-autostart >> ~/.gnupg/gpg-agent.conf
    ...
    # test sign with key: prompts for passphrase only if not already cached by
    # gpg-agent, or raises an error if key not found
    echo test_sign | gpg --clearsign -u "$key" -o /dev/null || return 1

    # prepare remote host for gpg forward, and get its agent socket path
    local remote_sock
    remote_sock=$(ssh "$user_at_host" \
        "$(declare -f init_gpg_remote); \
         init_gpg_remote $(gpg --export --armor "$key" | base64 -w0) > /dev/null 2>&1; \
         gpgconf --list-dirs agent-socket") || return 1

    unset -f init_gpg_remote

    # -R remote_socket:local_socket
    # local should be agent-extra-socket for restricted access, but it does not work...
    # single line: zsh does not word-split command substitutions, so callers must
    # split explicitly (see ssh.gpg_agent_fwd below)
    echo "-R $remote_sock:$(gpgconf --list-dirs agent-socket) -o StreamLocalBindUnlink=yes"
}

# usage: ssh.gpg_agent_fwd <user@host> <gpg key id/mail..> [extra ssh args...]
function ssh.gpg_agent_fwd
{
    local user_at_host=$1
    local key=$2

    if [[ -z $user_at_host || -z $key ]]; then
        echo "ssh.gpg_agent_fwd <user@host> <gpg key id/mail..> [ssh args...]" >&2
        return 1
    fi
    shift 2

    local opts
    opts=$(ssh.gpg_agent_fwd_get_options "$user_at_host" "$key") || return 1

    local -a ssh_opts
    if [[ -n $ZSH_VERSION ]]; then
        ssh_opts=(${=opts})
    else
        read -r -a ssh_opts <<< "$opts"
    fi

    ssh "${ssh_opts[@]}" "$user_at_host" "$@"
}
