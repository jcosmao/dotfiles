...
function ww {
    res=$(whence "$1")
    type $res | grep -q "shell function"
    [[ $? -eq 0 ]] && which $res || echo $res
    [[ ! -e "$res" ]] && type $res | grep -v 'not found'
}
...
{(
    ...
    # no cd: chpwd hooks (python.auto_venv) would recurse
    dir=$PWD
    while true; do
        proot=$(find "$dir" -maxdepth 1 -name $rootdir_pattern | grep .)
